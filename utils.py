...
def get_stddev(x, k_h, k_w): return 1 / np.sqrt(k_w * k_h * x.get_shape()[-1])


__all__ = ('show_all_variables', 'get_image', 'save_images', 'imread',
           'merge_images', 'merge', 'imsave', 'center_crop', 'transform',
           'inverse_transform', 'to_json', 'make_gif', 'visualize',
           'image_manifold_size')
...
def visualize(sess, dcgan, config, option):
    image_frame_dim = int(np.ceil(config.batch_size**.5))
    if option == 0:
